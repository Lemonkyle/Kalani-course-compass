...
- 年级、先修课以及主科年度负担均为可确认继续添加的提醒，允许转学、补课等例外。英语、数学、社会学习和科学按一年通常 1 学分提示，两个 0.5 学分课程合计为一个年度负担。不可重复课程的重复记录、不可用课程和课表总容量仍受保护。
- 课程详情中的年级按钮恢复添加、成功和移除动画，支持键盘、触屏与减少动态效果。课程详情、课表搜索、自定义课程和模板检查使用同一套规划提醒。
